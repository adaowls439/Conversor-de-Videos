...
def compress_and_trim_video(target_size_bytes, cut_duration, input_file, max_attempts, start_time, end_time, output_file, fps, scale_X, scale_Y, min_size_bytes, log_file):
...
def cut_video(input_file, output_file, start_time, end_time, log_file):
    # Registrar início do corte no log
    create_log_file(log_file, f"Iniciando corte de {input_file}")
    ...
    try:
        ...
        create_log_file(log_file, f"Corte realizado com sucesso: {input_file} -> {output_file}")

    except ffmpeg.Error as e:
        create_log_file(log_file, f"Erro ao cortar o vídeo: {e.stderr.decode('utf8')}")
